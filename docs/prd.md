...
- Activating or focusing an existing VS Code window, including by clicking its taskbar icon, must not trigger an automatic arrange run.
- Automatic new-window runs must not normalize Z order.
...
- If the app starts and more than one eligible VS Code window already exists, it must run a single startup recovery arrange that uses the current left-to-right screen order and also hide/show-syncs the Explorer preview order.
- That startup recovery arrange must also normalize Z order to match the same left-to-right order.
...
- If the app launches while multiple VS Code windows already exist, it performs one startup recovery arrange that follows the current left-to-right screen order and also hide/show-syncs the grouped preview order.
- During that startup recovery arrange, the window stacking order also matches the same left-to-right order, with the left-most window on top.
