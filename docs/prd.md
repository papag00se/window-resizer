...
Build a per-user Windows background app that stays running and lets the user rearrange eligible Visual Studio Code windows from the tray icon. The same layout must also be available on demand from the tray icon, and the app may observe window-open events to preserve ordering, but those events must not trigger layout runs.
...
- Observe when a new VS Code window appears so session ordering stays stable, but do not resize windows automatically.
- Expose a tray icon action that reapplies the layout on demand.
- Make left-clicking the tray icon behave the same as selecting `Arrange Now`.
...
- Use hide/show sequencing to push Explorer's grouped VS Code preview order toward that same sequence on `Arrange Now`.
...
- As a user, I want a left click on the tray icon to do the same thing as `Arrange Now`.
...
### FR-3 Session Ordering Observation

- The app may watch for new VS Code top-level windows becoming visible so it can learn the first-seen order for the current session.
- Shell events must not trigger a layout run.
- Repeated shell events for the same window must not change the recorded ordering.
- Activating or focusing an existing VS Code window, including by clicking its taskbar icon, must not change the recorded ordering or trigger a layout run.

### FR-4 Manual Trigger

- Left-clicking the tray icon must allow the user to apply the layout immediately, and selecting `Arrange Now` from the tray menu must do the same thing.
...
- After `Arrange Now` applies the final bounds, it must normalize Z order to match that same left-to-right order:
  - left-most window on top
  - next window directly beneath it
...
- Explorer/taskbar restart: the tray icon must reappear and ordering observation must continue without user action.

## Acceptance Criteria

- With the app running, opening a new VS Code window does not trigger a layout run.
- Clicking `Arrange Now` from the tray or left-clicking the tray icon performs the layout using the saved width or the clamped working-area width if smaller, and full working-area height.
...
- After `Arrange Now`, the window stacking order also matches that left-to-right order, with the left-most window on top.
- Changing `Window width (px)` in `Settings...` and clicking `Save` persists the new width and future arrange runs use it.
- After the app has observed VS Code windows opening during the current session, clicking `Arrange Now` produces a left-to-right order that matches that observed-open sequence.
- Windows that predate the app session are placed deterministically using process start time, then PID, then window handle.
- If the app launches while multiple VS Code windows already exist, it does not rearrange them until the user invokes `Arrange Now`.
